...
|---|---|---|---|
| **Apple Weather (WeatherKit)** | The latitude/longitude points along your planned route. Requests go through Apple's WeatherKit framework on your device; Apple states that WeatherKit requests are not associated with your Apple ID and are not used to identify you. No addresses, no identifiers from us. | The primary source of the forecast we score. Responses are cached on-device for the validity window Apple attaches to them. | Governed by [Apple's Privacy Policy](https://www.apple.com/legal/privacy/) and the [Apple Weather attribution and legal page](https://weatherkit.apple.com/legal-attribution.html). Attribution is shown in Settings → About. |
| **MET Norway** (the Norwegian Meteorological Institute, api.met.no) — *backup provider, contacted only if Apple Weather is unavailable or fails* | The latitude/longitude points along your planned route, truncated to 4 decimal places. Requests also carry a User-Agent header identifying the app (name, version, and a developer contact address) as MET's terms of service require — this identifies *the app*, not *you*. No timestamps, no addresses, no identifiers. | To return the forecast we score when Apple Weather cannot. MET's API is queried directly from your device; responses are cached on-device per MET's HTTP expiry headers. | Weather data is provided by MET Norway / Yr under the [CC BY 4.0 / NLOD licence](https://api.met.no/doc/License); see also their [Terms of Service](https://api.met.no/doc/TermsOfService). Attribution is shown in Settings → About. |
...
- **Transfers outside the EEA.** Weather requests are served by Apple
  Weather; Apple services may process data in the United States and
  other jurisdictions; Apple uses Standard Contractual Clauses. The
  MET Norway backup weather provider processes requests in Norway
  (EEA). TelemetryDeck is hosted in Germany. RevenueCat is US-based
  and operates under SCCs.
- **Retention.** §7.
- **Your rights.** §8.
